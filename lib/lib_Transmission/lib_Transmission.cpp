...
bool Transmission::ping(const SocketAddress& server)
{
    int ack = -1;
    TCPSocket clientTCP;
    clientTCP.set_timeout(message.TIMEOUT);
    if(eth_error("clientTCP_open", clientTCP.open(_eth)) == NSAPI_ERROR_OK)
        eth_error("clientTCP_connect", ack = clientTCP.connect(server)); // to connect less than 30sec Change LWIPStack.cpp netconn_set_nonblocking(s->conn, false) to true
    eth_error("clientTCP_close", clientTCP.close());
    return ack == NSAPI_ERROR_OK;
}

bool Transmission::ping(const string& server, const int& port)
{
    int ack = -1;
    if(!server.empty())
    {
        ...
        clientTCP.set_timeout(message.TIMEOUT);
        if(eth_error("clientTCP_open", clientTCP.open(_eth)) == NSAPI_ERROR_OK)
            eth_error("clientTCP_connect", ack = clientTCP.connect(SocketAddress(server.c_str(), port))); // to connect less than 30sec Change LWIPStack.cpp netconn_set_nonblocking(s->conn, false) to true
        eth_error("clientTCP_close", clientTCP.close());
    }
    return ack == NSAPI_ERROR_OK;
}

bool Transmission::set(const string& ssend, const SocketAddress& server)
{
    int ack = -1;
    TCPSocket clientTCP;
    clientTCP.set_timeout(message.TIMEOUT);
    if(eth_error("clientTCP_open", clientTCP.open(_eth)) == NSAPI_ERROR_OK)
        if(eth_error("clientTCP_connect", clientTCP.connect(server)) == NSAPI_ERROR_OK) // to connect less than 30sec Change LWIPStack.cpp netconn_set_nonblocking(s->conn, false) to true
            eth_error("clientTCP_send", ack = clientTCP.send(ssend.c_str(), ssend.size()));
    eth_error("clientTCP_close", clientTCP.close());
    return ack > NSAPI_ERROR_OK;
}

bool Transmission::set(const string& ssend, const string& server, const int& port)
{
    int ack = -1;
    if(!server.empty())
    {
        ...
        clientTCP.set_timeout(message.TIMEOUT);
        if(eth_error("clientTCP_open", clientTCP.open(_eth)) == NSAPI_ERROR_OK)
            if(eth_error("clientTCP_connect", clientTCP.connect(SocketAddress(server.c_str(), port))) == NSAPI_ERROR_OK) // to connect less than 30sec Change LWIPStack.cpp netconn_set_nonblocking(s->conn, false) to true
                eth_error("clientTCP_send", ack = clientTCP.send(ssend.c_str(), ssend.size()));
        eth_error("clientTCP_close", clientTCP.close());
    }
    return ack > NSAPI_ERROR_OK;
...
string Transmission::get(const string& ssend, const SocketAddress& server)
{
    char buffer[256] = {0};
    TCPSocket clientTCP;
    clientTCP.set_timeout(message.TIMEOUT);
...
bool Transmission::smtp(string mail, string from, string subject, string data, const char* server)
{
    ...
    if(eth_error("clientSMTP_open", clientSMTP.open(_eth)) == NSAPI_ERROR_OK)
    {
        ...
        eth_error("clientSMTP_close", clientSMTP.close());
        // ThisThread::sleep_for(1s);   // a gerer par le user
    }
    return ((code == "220250250250354250221") || (code == "220250250250354"));
    // surcharge _queue pour rien, a gere par l'utilisateur de Transmission si besoin
